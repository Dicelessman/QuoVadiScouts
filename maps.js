...
class MapsManager {
  ...
  addStructureMarkerWithFallback(struttura) {
    ...
    // Tentativo 1: Cerca altre strutture nella stessa città che hanno coordinate
    if (struttura.Luogo && window.strutture) {
      const sameCityStructures = window.strutture.filter(s =>
        s.Luogo === struttura.Luogo &&
        ((s.coordinate && s.coordinate.lat) || s.coordinate_lat) &&
        s.id !== struttura.id
      );

      if (sameCityStructures.length > 0) {
        // Calcola la media delle coordinate per quella città
        let sumLat = 0, sumLng = 0;
        sameCityStructures.forEach(s => {
          const sLat = s.coordinate_lat || s.coordinate.lat;
          const sLng = s.coordinate_lng || s.coordinate.lng;
          sumLat += sLat;
          sumLng += sLng;
        });
        lat = sumLat / sameCityStructures.length;
        lng = sumLng / sameCityStructures.length;
        isCityFallback = true;
        console.log(`📍 Fallback basato sulla città (${struttura.Luogo}) trovato tramite ${sameCityStructures.length} strutture`);
      }
    }
    ...
    // Tentativo 2: Fallback per provincia (usata come ultimo ricorso)
    if (!lat || !lng) {
    ...
    this.markerCluster.addLayer(marker);
    console.log(`📍 Marker di fallback (${isCityFallback ? 'CITTA' : 'PROV'}) aggiunto per: ${struttura.Struttura}`);
  }
